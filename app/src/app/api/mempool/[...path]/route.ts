...
export async function GET(
  ...
  if (
    request.url.startsWith('http://localhost:3000/api/mempool/testnet4/apiaddress/')
  ...
  if (
    request.url === 'http://localhost:3000/api/mempool/testnet4/apiv1/fees/recommended'
  ) {
    const specialUrl = 'http://localhost:3000/api/mempool/testnet4/api/v1/fees/recommended';
  ...
  const cpfpMatch = request.url.match(/^http:\/\/localhost:3000\/api\/mempool\/testnet4\/apiv1\/cpfp\/([a-f0-9]{64})(\?.*)?$/);
  if (cpfpMatch) {
